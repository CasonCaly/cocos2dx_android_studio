...
-(void)clear{
   ...
   self.delegate = nil;
    ...
    if(self.listFriend)
        [self.listFriend removeAllObjects];
}

-(void)addFriend:(FriendOC*)friendoc{
    if(nil == self.listFriend)
        self.listFriend = [[NSMutableArray alloc] init];
    [self.listFriend addObject:friendoc];
}
